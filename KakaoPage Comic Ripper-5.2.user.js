...
// @version      5.2
// @description  Downloads chapter images from KakaoPage concurrently.
// @author       ozler365
// @license      MIT
...
(function() {
    ...
    // DELAY_BETWEEN_IMAGES is no longer strictly needed for the concurrent method, 
    // but kept here to preserve your original structure/variables
    ...
    function findKakaoImages(obj, url) {
        ...
        // Path: viewer_data -> imagedownloaddata -> files (Updated for new API structure)
        const viewerData = searchObj?.viewer_data || searchObj?.viewerData;
        const imageDownloadData = viewerData?.imagedownloaddata || viewerData?.imageDownloadData;

        if (imageDownloadData?.files) {
            const files = imageDownloadData.files;

            // Extract title if available, otherwise fallback
            let title = viewerData?.title || document.title.split('-')[0].trim() || 'Kakao_Chapter';

            // Format and sort images
            const images = files.map((f, i) => ({
                // Handle possible property variations in Kakao's API
                url: f.secure_url || f.secureUrl || f.url || f.image_url || f.imageUrl,
                ord: f.no || f.order || f.sort_order || f.sortOrder || i
    ...
    XMLHttpRequest.prototype.open = function(method, url) {
        this.addEventListener('load', function() {
            try {
                // Now checking for the updated 'imagedownloaddata' key
                if (this.responseText.includes('imagedownloaddata') || this.responseText.includes('imageDownloadData')) {
    ...
    // Direct Download (GM_download) updated for concurrent downloads
    function handleDirectDownload(images, folderName) {
        let completed = 0;
        const total = images.length;
        
        // Track retries per specific image index so one failure doesn't reset another
        const retries = new Array(total).fill(0);

        statusText.innerText = `Starting ${total} downloads...`;

        function checkFinished() {
            if (completed >= total) {
                statusText.innerText = `✓ Done ${completed}/${total}`;
        ...
        function downloadImage(index) {
            ...
            const handleFailure = () => {
                if (retries[index] < MAX_RETRIES) {
                    ...
                    // Mark as complete even if failed, so the UI still finishes
                    completed++; 
                    statusText.innerText = `Folder DL: ${completed} / ${total}`;
                    checkFinished();
                }
            };
